...
def foldspectrum(archives):
    ...
    for f in archives:
        i += 1
        print ("loading %s %s" % (f, i))
        I, f = readpsrfits(f, undo_scaling=True, dedisperse=True)
        if switch:
            Ic = np.copy(I)
            F = np.copy(f)
            switch = 0
...
def getdynspec(foldspec, profsig=5., sigma=10.):
    """
...
def standard_psrfits(args):
    ...
    while number < len(archives_segmentation):
        ...
        #remove RFI
        foldspec, flag, mask = clean_foldspec(foldspec, plots=False, apply_mask=True, rfimethod='var')
        #get dynamic spectrum
        dynspec = getdynspec(foldspec, profsig=5., sigma=10.)
